...
def analyze_document(file_obj, doc_type_hint=None):
    ...
    for i, key in enumerate(api_keys):
        try:
            ...
            # On utilise le nom générique qui pointe vers la version stable
            # On ajoute aussi le flash-lite/8b en secours
            ...
            for model_name in models_to_try:
                try:
                    ...
                    # On continue sur le modèle suivant si c'est un quota ou un 404 (parfois le nom change)
                    if "429" in last_error or "quota" in last_error.lower() or "not found" in last_error.lower() or "404" in last_error:
                        continue 
                    else:
                        break # On passe à la clé suivante pour d'autres types d'erreurs
    ...
    with transaction.atomic():
        ...
        if doc_type == "DEVIS_CLIENT" and client:
            ...
            result_obj = Commande.objects.create(
                ...
                designation=f"Commande {refs.get('num_commande') or 'Inconnue'}", # Ensure designation is set
            ...
            for l in lignes:
                ...
                # Truncate designation to 255 chars to avoid DB error
                ...
                art, _ = Article.objects.get_or_create(
                    ...
                    defaults={'designation': des, 'fournisseur': fournisseur, 'prix_unitaire_ht': l.get('prix_unitaire', 0)}
                )
            ...
            if fournisseur:
                for l in lignes:
                    ...
                    if art_code:
                        # Truncate designation to 255 chars
                        des = l.get('designation') or "Article Sans Nom"
                        if len(des) > 255: des = des[:252] + "..."
